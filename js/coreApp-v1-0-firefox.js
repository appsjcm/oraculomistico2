...
function getAstroPdfChart(reading = lastReading) {
  const meta = reading?.meta || {};
  return meta.solarReturn?.chart || meta.astroPeriod?.chart || meta.astroToday || meta.astro || null;
}
...
function showGuide(force = false) {
  ...
  openModal({
    ...
    body: `
      ...
      <div class="field mt"><label for="guideName">${escapeHTML(t('guideName'))}</label>${inputWithMic('guideName', `value="${currentName}" placeholder="${escapeHTML(t('guideNamePh'))}"`)}</div>
...
function showGlobalSearch() { openModal({ icon:'🔎', title:t('mdBuscar'), body:`<div class="field"><label for="globalSearchInput">${escapeHTML(t('stBuscarModulo'))}</label><input id="globalSearchInput" class="input" placeholder="Tarot, luna, astros, biblioteca..."></div><div id="globalSearchResults" class="diary-list mt">${renderGlobalSearchResults()}</div>` }); }
...
function showMiOraculo() {
  ...
  openModal({ icon:'🪬', title:t('mdMiOraculo'), subtitle:t('mdMiOraculoS'), body:`
    ...
    <div class="form-grid mt">
      <div class="field"><label for="profileBirth">${escapeHTML(t('stFechaDeNacimiento'))}</label><input class="input" id="profileBirth" type="date" value="${escapeHTML(profile.birth || '')}"></div>
      <div class="field"><label for="profileBirthTime">${escapeHTML(t('stHoraDeNacimiento'))}</label><input class="input" id="profileBirthTime" type="time" value="${escapeHTML(profile.birthTime || '')}"></div>
      <div class="field"><label for="profileBirthPlace">${escapeHTML(t('stLugarDeNacimiento'))}</label><input class="input" id="profileBirthPlace" value="${escapeHTML(profile.birthPlace?.label || '')}" placeholder="${escapeHTML(t('phCiudadPais'))}"></div>
      <div class="field"><label for="profileSign">${escapeHTML(t('stSignoEnergia'))}</label><input class="input" id="profileSign" value="${escapeHTML(profile.sign || '')}" placeholder="Aries, Luna, Agua..."></div>
      <div class="field"><label for="profileIntention">${escapeHTML(t('stIntencionPrincipal'))}</label><input class="input" id="profileIntention" value="${escapeHTML(profile.intention || '')}" placeholder="Claridad, amor, calma..."></div>
      <div class="field"><label for="profileSpread">${escapeHTML(t('stTiradaFavorita'))}</label><select id="profileSpread"><option value="one" ${profile.favoriteSpread==='one'?'selected':''}>${escapeHTML(t('stCartaRapida'))}</option><option value="three" ${(profile.favoriteSpread||'three')==='three'?'selected':''}>${escapeHTML(t('stPasadoPresenteFuturo'))}</option><option value="love" ${profile.favoriteSpread==='love'?'selected':''}>${escapeHTML(t('stAmor'))}</option><option value="decision" ${profile.favoriteSpread==='decision'?'selected':''}>${escapeHTML(t('stDecision'))}</option><option value="celtic" ${profile.favoriteSpread==='celtic'?'selected':''}>${escapeHTML(t('stCruzCelta'))}</option></select></div>
    </div>
...
function appHealthCheck() {
  ...
  openModal({ icon:'🧪', title:t('mdEstado'), subtitle:getAppVersionLabel(), body:`
    <div class="status-grid">
      ...
      <div class="status-card"><strong>IA</strong><span>${escapeHTML(t(localStorage.getItem(LS.puter)==='true'?'stIaConectada':'stIaSimbolica'))}</span></div>
...
function showNumerologia() {
  ...
  openModal({ icon:'🔢', title:t('nuTitle'), subtitle:t('nuSub'), body:`
    ...
    <div class="synastry-grid mt">
      <div class="result-card"><h3>${escapeHTML(t('nuPersonA'))}</h3><div class="field"><label for="synNameA">${escapeHTML(t('nuName'))}</label><input id="synNameA" class="input" value="${n}" placeholder="${escapeHTML(t('nuNamePh'))}"></div><div class="field mt"><label for="synDateA">${escapeHTML(t('nuBirth'))}</label><input id="synDateA" class="input" type="date" value="${d}"></div><div class="field mt"><label for="synTimeA">${escapeHTML(t('nuBirthTime'))}</label><input id="synTimeA" class="input" type="time" value="${tm}"></div></div>
      <div class="result-card"><h3>${escapeHTML(t('nuPersonB'))}</h3><div class="field"><label for="synNameB">${escapeHTML(t('nuName'))}</label><input id="synNameB" class="input" placeholder="${escapeHTML(t('nuNamePh'))}"></div><div class="field mt"><label for="synDateB">${escapeHTML(t('nuBirth'))}</label><input id="synDateB" class="input" type="date"></div><div class="field mt"><label for="synTimeB">${escapeHTML(t('nuBirthTime'))}</label><input id="synTimeB" class="input" type="time"></div></div>
    </div>
...
/* Efemerides mensuales de Quiron, 1800 a 2120.

   Empezo cubriendo solo de 1975 a 1980, seis anios, y se amplio a
   1900-2075. Pero seguia habiendo fechas fuera: probando con un
   nacimiento en 1850 la carta salia igual y Quiron aparecia al segundo
   de arco, sin que nada dijera que ese dato venia del respaldo malo. Un
   numero que aparenta precision y no la tiene es peor que no darlo. Con
   1800-2120 dentro ya no queda fecha realista fuera, y cuesta seis
   kilobytes comprimidos.
...
const CHIRON_MONTHLY_EPHEMERIS = {
  1800:['21 sc 16','23 sc 54','24 sc 57','24 sc 29 rx','22 sc 42 rx','20 sc 26 rx','18 sc 55 rx','18 sc 46','20 sc 13','22 sc 53','26 sc 31','0 sa 21'],
  1801:['4 sa 7','7 sa 6','8 sa 41','8 sa 55 rx','7 sa 42 rx','5 sa 35 rx','3 sa 40 rx','2 sa 46 rx','3 sa 22','5 sa 18','8 sa 21','11 sa 51'],
  1802:['15 sa 30','18 sa 38','20 sa 34','21 sa 21','20 sa 40 rx','18 sa 52 rx','16 sa 51 rx','15 sa 27 rx','15 sa 20','16 sa 35','19 sa 3','22 sa 9'],
  1803:['25 sa 36','28 sa 44','0 cp 52','2 cp 3','1 cp 51 rx','0 cp 26 rx','28 sa 29 rx','26 sa 47 rx','26 sa 8','26 sa 48','28 sa 42','1 cp 24'],
  1804:['4 cp 35','7 cp 39','9 cp 57','11 cp 24','11 cp 32 rx','10 cp 27 rx','8 cp 40 rx','6 cp 51 rx','5 cp 50 rx','6 cp 3','7 cp 30','9 cp 50'],
  1805:['12 cp 45','15 cp 42','17 cp 57','19 cp 35','20 cp 2 rx','19 cp 17 rx','17 cp 42 rx','15 cp 51 rx','14 cp 35 rx','14 cp 24','15 cp 25','17 cp 23'],
  1806:['20 cp 2','22 cp 50','25 cp 6','26 cp 52','27 cp 33','27 cp 6 rx','25 cp 44 rx','23 cp 56 rx','22 cp 30 rx','22 cp 0','22 cp 38','24 cp 16'],
  1807:['26 cp 40','29 cp 19','1 aq 33','3 aq 25','4 aq 18','4 aq 6 rx','2 aq 57 rx','1 aq 14 rx','29 cp 42 rx','28 cp 57 rx','29 cp 16','0 aq 36'],
  1808:['2 aq 45','5 aq 16','7 aq 32','9 aq 26','10 aq 26','10 aq 25 rx','9 aq 26 rx','7 aq 49 rx','6 aq 15 rx','5 aq 20 rx','5 aq 26','6 aq 31'],
  1809:['8 aq 28','10 aq 50','12 aq 58','14 aq 54','16 aq 2','16 aq 12 rx','15 aq 25 rx','13 aq 55 rx','12 aq 19 rx','11 aq 16 rx','11 aq 8','11 aq 59'],
  1810:['13 aq 43','15 aq 56','18 aq 1','19 aq 58','21 aq 12','21 aq 32 rx','20 aq 55 rx','19 aq 33 rx','17 aq 58 rx','16 aq 48 rx','16 aq 28','17 aq 6'],
  1811:['18 aq 38','20 aq 44','22 aq 44','24 aq 43','26 aq 1','26 aq 30','26 aq 3 rx','24 aq 48 rx','23 aq 15 rx','22 aq 0 rx','21 aq 30 rx','21 aq 56'],
  1812:['23 aq 18','25 aq 15','27 aq 16','29 aq 14','0 pi 35','1 pi 10','0 pi 50 rx','29 aq 42 rx','28 aq 10 rx','26 aq 53 rx','26 aq 15 rx','26 aq 34'],
  1813:['27 aq 46','29 aq 37','1 pi 30','3 pi 28','4 pi 53','5 pi 35','5 pi 23 rx','4 pi 22 rx','2 pi 53 rx','1 pi 34 rx','0 pi 48 rx','0 pi 57'],
  1814:['2 pi 0','3 pi 43','5 pi 33','7 pi 30','8 pi 59','9 pi 47','9 pi 42 rx','8 pi 48 rx','7 pi 24 rx','6 pi 2 rx','5 pi 10 rx','5 pi 10'],
  1815:['6 pi 4','7 pi 41','9 pi 27','11 pi 24','12 pi 55','13 pi 48','13 pi 51 rx','13 pi 4 rx','11 pi 43 rx','10 pi 21 rx','9 pi 23 rx','9 pi 15'],
  1816:['10 pi 1','11 pi 31','13 pi 17','15 pi 13','16 pi 45','17 pi 43','17 pi 51 rx','17 pi 9 rx','15 pi 52 rx','14 pi 29 rx','13 pi 27 rx','13 pi 13'],
  1817:['13 pi 53','15 pi 18','16 pi 57','18 pi 52','20 pi 27','21 pi 30','21 pi 44 rx','21 pi 9 rx','19 pi 56 rx','18 pi 33 rx','17 pi 27 rx','17 pi 6'],
  1818:['17 pi 38','18 pi 57','20 pi 33','22 pi 27','24 pi 4','25 pi 11','25 pi 32 rx','25 pi 4 rx','23 pi 55 rx','22 pi 32 rx','21 pi 22 rx','20 pi 55 rx'],
  1819:['21 pi 19','22 pi 32','24 pi 5','25 pi 58','27 pi 37','28 pi 49','29 pi 16','28 pi 54 rx','27 pi 49 rx','26 pi 28 rx','25 pi 14 rx','24 pi 41 rx'],
  1820:['24 pi 58','26 pi 5','27 pi 38','29 pi 30','1 ar 10','2 ar 25','2 ar 57','2 ar 40 rx','1 ar 39 rx','0 ar 18 rx','29 pi 2 rx','28 pi 24 rx'],
  1821:['28 pi 36','29 pi 38','1 ar 5','2 ar 56','4 ar 38','5 ar 58','6 ar 36','6 ar 25 rx','5 ar 29 rx','4 ar 9 rx','2 ar 51 rx','2 ar 7 rx'],
  1822:['2 ar 12','3 ar 9','4 ar 32','6 ar 22','8 ar 6','9 ar 30','10 ar 13','10 ar 10 rx','9 ar 19 rx','8 ar 1 rx','6 ar 40 rx','5 ar 50 rx'],
  1823:['5 ar 49','6 ar 39','7 ar 59','9 ar 48','11 ar 34','13 ar 2','13 ar 51','13 ar 54 rx','13 ar 8 rx','11 ar 52 rx','10 ar 29 rx','9 ar 35 rx'],
  1824:['9 ar 26','10 ar 11','11 ar 31','13 ar 19','15 ar 6','16 ar 37','17 ar 31','17 ar 39 rx','16 ar 58 rx','15 ar 43 rx','14 ar 19 rx','13 ar 20 rx'],
  1825:['13 ar 6','13 ar 47','15 ar 1','16 ar 48','18 ar 36','20 ar 12','21 ar 12','21 ar 27 rx','20 ar 51 rx','19 ar 40 rx','18 ar 14 rx','17 ar 10 rx'],
  1826:['16 ar 50','17 ar 23','18 ar 34','20 ar 20','22 ar 10','23 ar 50','24 ar 56','25 ar 18 rx','24 ar 49 rx','23 ar 41 rx','22 ar 14 rx','21 ar 5 rx'],
  1827:['20 ar 37 rx','21 ar 5','22 ar 11','23 ar 56','25 ar 47','27 ar 32','28 ar 44','29 ar 14','28 ar 52 rx','27 ar 48 rx','26 ar 20 rx','25 ar 6 rx'],
  1828:['24 ar 31 rx','24 ar 52','25 ar 57','27 ar 41','29 ar 34','1 ta 22','2 ta 40','3 ta 17','3 ta 0 rx','1 ta 59 rx','0 ta 31 rx','29 ar 13 rx'],
  1829:['28 ar 33 rx','28 ar 48','29 ar 47','1 ta 29','3 ta 24','5 ta 17','6 ta 42','7 ta 27','7 ta 19 rx','6 ta 22 rx','4 ta 54 rx','3 ta 32 rx'],
  1830:['2 ta 44 rx','2 ta 52','3 ta 46','5 ta 26','7 ta 22','9 ta 20','10 ta 52','11 ta 47','11 ta 47 rx','10 ta 57 rx','9 ta 30 rx','8 ta 3 rx'],
  1831:['7 ta 7 rx','7 ta 7','7 ta 56','9 ta 33','11 ta 31','13 ta 34','15 ta 14','16 ta 18','16 ta 28 rx','15 ta 45 rx','14 ta 20 rx','12 ta 49 rx'],
  1832:['11 ta 45 rx','11 ta 36','12 ta 21','13 ta 57','15 ta 56','18 ta 4','19 ta 51','21 ta 4','21 ta 24 rx','20 ta 47 rx','19 ta 24 rx','17 ta 50 rx'],
  1833:['16 ta 39 rx','16 ta 22','17 ta 0','18 ta 32','20 ta 32','22 ta 45','24 ta 41','26 ta 6','26 ta 38','26 ta 11 rx','24 ta 52 rx','23 ta 15 rx'],
  1834:['21 ta 55 rx','21 ta 27','21 ta 58','23 ta 25','25 ta 26','27 ta 45','29 ta 50','1 ge 27','2 ge 13','1 ge 57 rx','0 ge 45 rx','29 ta 6 rx'],
  1835:['27 ta 37 rx','26 ta 57 rx','27 ta 18','28 ta 41','0 ge 41','3 ge 5','5 ge 20','7 ge 11','8 ge 12','8 ge 11 rx','7 ge 7 rx','5 ge 28 rx'],
  1836:['3 ge 50 rx','2 ge 57 rx','3 ge 9','4 ge 26','6 ge 27','8 ge 56','11 ge 20','13 ge 25','14 ge 43','14 ge 56 rx','14 ge 2 rx','12 ge 24 rx'],
  1837:['10 ge 37 rx','9 ge 32 rx','9 ge 32','10 ge 40','12 ge 39','15 ge 13','17 ge 48','20 ge 9','21 ge 46','22 ge 20 rx','21 ge 42 rx','20 ge 9 rx'],
  1838:['18 ge 15 rx','16 ge 53 rx','16 ge 38','17 ge 35','19 ge 29','22 ge 6','24 ge 52','27 ge 31','29 ge 31','0 cn 28','0 cn 11 rx','28 ge 48 rx'],
  1839:['26 ge 49 rx','25 ge 11 rx','24 ge 38','25 ge 19','27 ge 6','29 ge 45','2 cn 41','5 cn 38','8 cn 3','9 cn 28','9 cn 39 rx','8 cn 33 rx'],
  1840:['6 cn 34 rx','4 cn 39 rx','3 cn 45 rx','4 cn 7','5 cn 46','8 cn 25','11 cn 30','14 cn 45','17 cn 36','19 cn 31','20 cn 14 rx','19 cn 31 rx'],
  1841:['17 cn 39 rx','15 cn 31 rx','14 cn 15 rx','14 cn 11','15 cn 32','18 cn 6','21 cn 16','24 cn 48','28 cn 7','0 le 38','2 le 3','1 le 57 rx'],
  1842:['0 le 24 rx','28 cn 9 rx','26 cn 29 rx','25 cn 52','26 cn 47','29 cn 6','2 le 17','6 le 2','9 le 48','12 le 56','15 le 8','15 le 50 rx'],
  1843:['14 le 54 rx','12 le 46 rx','10 le 44 rx','9 le 28 rx','9 le 47','11 le 41','14 le 42','18 le 34','22 le 40','26 le 23','29 le 24','1 vi 2'],
  1844:['0 vi 59 rx','29 le 19 rx','27 le 5 rx','25 le 12 rx','24 le 47','26 le 5','28 le 45','2 vi 31','6 vi 48','10 vi 57','14 vi 41','17 vi 13'],
  1845:['18 vi 11','17 vi 19 rx','15 vi 22 rx','13 vi 1 rx','11 vi 45 rx','12 vi 8','14 vi 8','17 vi 31','21 vi 43','26 vi 4','0 li 20','3 li 41'],
  1846:['5 li 45','5 li 58 rx','4 li 39 rx','2 li 16 rx','0 li 17 rx','29 vi 38','0 li 42','3 li 22','7 li 10','11 li 26','15 li 56','19 li 49'],
  1847:['22 li 46','24 li 5','23 li 39 rx','21 li 44 rx','19 li 25 rx','17 li 51 rx','17 li 51','19 li 33','22 li 40','26 li 34','0 sc 58','5 sc 5'],
  1848:['8 sc 36','10 sc 46','11 sc 13 rx','10 sc 2 rx','7 sc 52 rx','5 sc 45 rx','4 sc 51 rx','5 sc 33','7 sc 50','11 sc 9','15 sc 13','19 sc 17'],
  1849:['23 sc 1','25 sc 44','26 sc 52','26 sc 29 rx','24 sc 46 rx','22 sc 30 rx','20 sc 55 rx','20 sc 40','22 sc 1','24 sc 37','28 sc 11','2 sa 1'],
  1850:['5 sa 47','8 sa 48','10 sa 27','10 sa 46 rx','9 sa 38 rx','7 sa 32 rx','5 sa 36 rx','4 sa 37 rx','5 sa 7','6 sa 59','9 sa 58','13 sa 26'],
  1851:['17 sa 5','20 sa 14','22 sa 12','23 sa 4','22 sa 28 rx','20 sa 42 rx','18 sa 41 rx','17 sa 13 rx','17 sa 2','18 sa 13','20 sa 36','23 sa 40'],
  1852:['27 sa 6','0 cp 14','2 cp 28','3 cp 40','3 cp 29 rx','2 cp 5 rx','0 cp 9 rx','28 sa 26 rx','27 sa 46 rx','28 sa 24','0 cp 17','2 cp 57'],
  1853:['6 cp 7','9 cp 10','11 cp 24','12 cp 54','13 cp 5 rx','12 cp 3 rx','10 cp 17 rx','8 cp 27 rx','7 cp 24 rx','7 cp 33','8 cp 56','11 cp 13'],
  1854:['14 cp 7','17 cp 3','19 cp 19','21 cp 0','21 cp 30 rx','20 cp 48 rx','19 cp 14 rx','17 cp 24 rx','16 cp 5 rx','15 cp 51','16 cp 48','18 cp 43'],
  1855:['21 cp 21','24 cp 9','26 cp 25','28 cp 13','28 cp 57','28 cp 32 rx','27 cp 12 rx','25 cp 25 rx','23 cp 56 rx','23 cp 24','23 cp 59','25 cp 34'],
  1856:['27 cp 56','0 aq 35','2 aq 53','4 aq 45','5 aq 38','5 aq 26 rx','4 aq 18 rx','2 aq 36 rx','1 aq 3 rx','0 aq 18 rx','0 aq 36','1 aq 56'],
  1857:['4 aq 4','6 aq 34','8 aq 45','10 aq 40','11 aq 42','11 aq 43 rx','10 aq 47 rx','9 aq 11 rx','7 aq 35 rx','6 aq 39 rx','6 aq 42','7 aq 45'],
  1858:['9 aq 40','12 aq 1','14 aq 8','16 aq 6','17 aq 15','17 aq 27 rx','16 aq 42 rx','15 aq 14 rx','13 aq 37 rx','12 aq 33 rx','12 aq 22','13 aq 11'],
  1859:['14 aq 53','17 aq 5','19 aq 9','21 aq 8','22 aq 23','22 aq 45 rx','22 aq 10 rx','20 aq 49 rx','19 aq 14 rx','18 aq 3 rx','17 aq 40','18 aq 16'],
  1860:['19 aq 47','21 aq 51','23 aq 56','25 aq 54','27 aq 12','27 aq 41','27 aq 14 rx','25 aq 59 rx','24 aq 26 rx','23 aq 11 rx','22 aq 40 rx','23 aq 7'],
  1861:['24 aq 28','26 aq 25','28 aq 22','0 pi 20','1 pi 43','2 pi 19','2 pi 1 rx','0 pi 54 rx','29 aq 23 rx','28 aq 5 rx','27 aq 25 rx','27 aq 41'],
  1862:['28 aq 52','0 pi 42','2 pi 35','4 pi 33','5 pi 59','6 pi 42','6 pi 32 rx','5 pi 32 rx','4 pi 5 rx','2 pi 44 rx','1 pi 57 rx','2 pi 4'],
  1863:['3 pi 5','4 pi 48','6 pi 37','8 pi 35','10 pi 4','10 pi 53','10 pi 51 rx','9 pi 58 rx','8 pi 34 rx','7 pi 12 rx','6 pi 18 rx','6 pi 17'],
  1864:['7 pi 9','8 pi 45','10 pi 34','12 pi 31','14 pi 2','14 pi 55','14 pi 58 rx','14 pi 11 rx','12 pi 50 rx','11 pi 28 rx','10 pi 30 rx','10 pi 22'],
  1865:['11 pi 8','12 pi 38','14 pi 20','16 pi 16','17 pi 49','18 pi 48','18 pi 58 rx','18 pi 18 rx','17 pi 1 rx','15 pi 38 rx','14 pi 35 rx','14 pi 20'],
  1866:['14 pi 57','16 pi 21','18 pi 0','19 pi 55','21 pi 31','22 pi 35','22 pi 51 rx','22 pi 17 rx','21 pi 5 rx','19 pi 42 rx','18 pi 35 rx','18 pi 12'],
  1867:['18 pi 42','20 pi 0','21 pi 35','23 pi 29','25 pi 7','26 pi 16','26 pi 38 rx','26 pi 11 rx','25 pi 3 rx','23 pi 41 rx','22 pi 30 rx','22 pi 1 rx'],
  1868:['22 pi 24','23 pi 35','25 pi 11','27 pi 4','28 pi 43','29 pi 55','0 ar 22','0 ar 0 rx','28 pi 55 rx','27 pi 33 rx','26 pi 20 rx','25 pi 46 rx'],
  1869:['26 pi 4','27 pi 10','28 pi 40','0 ar 32','2 ar 13','3 ar 29','4 ar 2','3 ar 47 rx','2 ar 47 rx','1 ar 26 rx','0 ar 10 rx','29 pi 30 rx'],
  1870:['29 pi 40','0 ar 41','2 ar 7','3 ar 58','5 ar 41','7 ar 2','7 ar 41','7 ar 32 rx','6 ar 37 rx','5 ar 17 rx','3 ar 58 rx','3 ar 13 rx'],
  1871:['3 ar 16','4 ar 11','5 ar 34','7 ar 24','9 ar 8','10 ar 33','11 ar 18','11 ar 16 rx','10 ar 26 rx','9 ar 8 rx','7 ar 47 rx','6 ar 56 rx'],
  1872:['6 ar 53','7 ar 42','9 ar 4','10 ar 53','12 ar 39','14 ar 7','14 ar 56','14 ar 59 rx','14 ar 13 rx','12 ar 57 rx','11 ar 34 rx','10 ar 39 rx'],
  1873:['10 ar 31','11 ar 15','12 ar 32','14 ar 20','16 ar 7','17 ar 40','18 ar 35','18 ar 45 rx','18 ar 5 rx','16 ar 51 rx','15 ar 26 rx','14 ar 26 rx'],
  1874:['14 ar 11','14 ar 49','16 ar 3','17 ar 49','19 ar 38','21 ar 15','22 ar 16','22 ar 33 rx','21 ar 59 rx','20 ar 48 rx','19 ar 22 rx','18 ar 17 rx'],
  1875:['17 ar 54','18 ar 27','19 ar 36','21 ar 22','23 ar 12','24 ar 53','26 ar 0','26 ar 25 rx','25 ar 57 rx','24 ar 50 rx','23 ar 22 rx','22 ar 12 rx'],
  1876:['21 ar 43 rx','22 ar 9','23 ar 17','25 ar 2','26 ar 54','28 ar 39','29 ar 51','0 ta 21','29 ar 59 rx','28 ar 55 rx','27 ar 27 rx','26 ar 13 rx'],
  1877:['25 ar 38 rx','25 ar 59','27 ar 2','28 ar 44','0 ta 38','2 ta 27','3 ta 47','4 ta 25','4 ta 10 rx','3 ta 10 rx','1 ta 42 rx','0 ta 24 rx'],
  1878:['29 ar 41 rx','29 ar 55','0 ta 53','2 ta 34','4 ta 29','6 ta 23','7 ta 50','8 ta 37','8 ta 30 rx','7 ta 35 rx','6 ta 7 rx','4 ta 45 rx'],
  1879:['3 ta 55 rx','4 ta 1','4 ta 54','6 ta 33','8 ta 29','10 ta 28','12 ta 2','12 ta 58','13 ta 0 rx','12 ta 12 rx','10 ta 45 rx','9 ta 18 rx'],
  1880:['8 ta 20 rx','8 ta 18','9 ta 8','10 ta 46','12 ta 44','14 ta 47','16 ta 28','17 ta 32','17 ta 43 rx','17 ta 0 rx','15 ta 35 rx','14 ta 4 rx'],
  1881:['12 ta 59 rx','12 ta 50','13 ta 33','15 ta 8','17 ta 7','19 ta 15','21 ta 4','22 ta 19','22 ta 41 rx','22 ta 7 rx','20 ta 45 rx','19 ta 10 rx'],
  1882:['17 ta 57 rx','17 ta 38','18 ta 14','19 ta 45','21 ta 45','23 ta 59','25 ta 56','27 ta 23','27 ta 58','27 ta 33 rx','26 ta 16 rx','24 ta 39 rx'],
  1883:['23 ta 16 rx','22 ta 46 rx','23 ta 15','24 ta 41','26 ta 41','29 ta 0','1 ge 7','2 ge 47','3 ge 35','3 ge 22 rx','2 ge 12 rx','0 ge 33 rx'],
  1884:['29 ta 2 rx','28 ta 19 rx','28 ta 40','0 ge 2','2 ge 3','4 ge 28','6 ge 44','8 ge 36','9 ge 39','9 ge 38 rx','8 ge 35 rx','6 ge 55 rx'],
  1885:['5 ge 16 rx','4 ge 22 rx','4 ge 33','5 ge 48','7 ge 48','10 ge 17','12 ge 43','14 ge 50','16 ge 11','16 ge 28 rx','15 ge 37 rx','14 ge 0 rx'],
  1886:['12 ge 12 rx','11 ge 3 rx','11 ge 1','12 ge 6','14 ge 4','16 ge 38','19 ge 14','21 ge 38','23 ge 19','23 ge 56','23 ge 22 rx','21 ge 50 rx'],
  1887:['19 ge 55 rx','18 ge 31 rx','18 ge 13','19 ge 6','20 ge 59','23 ge 36','26 ge 23','29 ge 4','1 cn 8','2 cn 10','1 cn 57 rx','0 cn 37 rx'],
  1888:['28 ge 38 rx','26 ge 57 rx','26 ge 20','27 ge 0','28 ge 47','1 cn 27','4 cn 24','7 cn 23','9 cn 51','11 cn 18','11 cn 32 rx','10 cn 27 rx'],
  1889:['8 cn 28 rx','6 cn 32 rx','5 cn 36 rx','5 cn 55','7 cn 30','10 cn 8','13 cn 14','16 cn 31','19 cn 26','21 cn 27','22 cn 16','21 cn 38 rx'],
  1890:['19 cn 49 rx','17 cn 39 rx','16 cn 19 rx','16 cn 11','17 cn 28','19 cn 59','23 cn 9','26 cn 43','0 le 6','2 le 43','4 le 15','4 le 16 rx'],
  1891:['2 le 48 rx','0 le 33 rx','28 cn 49 rx','28 cn 6','28 cn 56','1 le 12','4 le 21','8 le 8','11 le 56','15 le 10','17 le 30','18 le 20'],
  1892:['17 le 32 rx','15 le 27 rx','13 le 19 rx','11 le 59 rx','12 le 14','14 le 6','17 le 6','20 le 58','25 le 6','28 le 52','1 vi 59','3 vi 43'],
  1893:['3 vi 46 rx','2 vi 11 rx','0 vi 1 rx','28 le 3 rx','27 le 30','28 le 40','1 vi 15','4 vi 58','9 vi 15','13 vi 27','17 vi 16','19 vi 57'],
  1894:['21 vi 6','20 vi 24 rx','18 vi 31 rx','16 vi 8 rx','14 vi 45 rx','14 vi 58','16 vi 50','20 vi 7','24 vi 17','28 vi 39','2 li 58','6 li 25'],
  1895:['8 li 38','9 li 2 rx','7 li 50 rx','5 li 30 rx','3 li 27 rx','2 li 38','3 li 32','6 li 4','9 li 47','14 li 1','18 li 30','22 li 27'],
  1896:['25 li 30','26 li 58','26 li 39 rx','24 li 48 rx','22 li 30 rx','20 li 51 rx','20 li 45','22 li 21','25 li 22','29 li 12','3 sc 34','7 sc 41'],
  1897:['11 sc 13','13 sc 27','13 sc 59 rx','12 sc 55 rx','10 sc 49 rx','8 sc 39 rx','7 sc 38 rx','8 sc 11','10 sc 18','13 sc 31','17 sc 31','21 sc 32'],
  1898:['25 sc 18','28 sc 4','29 sc 18','29 sc 2 rx','27 sc 24 rx','25 sc 10 rx','23 sc 31 rx','23 sc 8','24 sc 21','26 sc 49','0 sa 18','4 sa 4'],
  1899:['7 sa 49','10 sa 52','12 sa 34','12 sa 59 rx','11 sa 56 rx','9 sa 54 rx','7 sa 57 rx','6 sa 52 rx','7 sa 16','9 sa 0','11 sa 54','15 sa 17'],
  ...
  2075:['18 ar 6','18 ar 37','19 ar 44','21 ar 26','23 ar 12','24 ar 48','25 ar 52','26 ar 13 rx','25 ar 45 rx','24 ar 39 rx','23 ar 14 rx','22 ar 7'],
  2076:['21 ar 40 rx','22 ar 5','23 ar 11','24 ar 52','26 ar 39','28 ar 18','29 ar 26','29 ar 53','29 ar 29 rx','28 ar 26 rx','27 ar 1 rx','25 ar 50 rx'],
  2077:['25 ar 18 rx','25 ar 38','26 ar 39','28 ar 18','0 ta 6','1 ta 50','3 ta 4','3 ta 37','3 ta 20 rx','2 ta 20 rx','0 ta 55 rx','29 ar 41 rx'],
  2078:['29 ar 2 rx','29 ar 16','0 ta 12','1 ta 50','3 ta 39','5 ta 27','6 ta 46','7 ta 27','7 ta 17 rx','6 ta 22 rx','4 ta 57 rx','3 ta 39 rx'],
  2079:['2 ta 53 rx','3 ta 0','3 ta 52','5 ta 28','7 ta 18','9 ta 10','10 ta 36','11 ta 25','11 ta 22 rx','10 ta 32 rx','9 ta 8 rx','7 ta 46 rx'],
  2080:['6 ta 53 rx','6 ta 53','7 ta 43','9 ta 17','11 ta 9','13 ta 4','14 ta 36','15 ta 32','15 ta 36 rx','14 ta 50 rx','13 ta 27 rx','12 ta 2 rx'],
  2081:['11 ta 3 rx','10 ta 58','11 ta 41','13 ta 12','15 ta 5','17 ta 5','18 ta 44','19 ta 49','20 ta 2 rx','19 ta 23 rx','18 ta 2 rx','16 ta 33 rx'],
  2082:['15 ta 27 rx','15 ta 13','15 ta 51','17 ta 19','19 ta 13','21 ta 17','23 ta 3','24 ta 18','24 ta 41 rx','24 ta 10 rx','22 ta 52 rx','21 ta 21 rx'],
  2083:['20 ta 7 rx','19 ta 44','20 ta 16','21 ta 40','23 ta 34','25 ta 43','27 ta 36','29 ta 2','29 ta 36','29 ta 14 rx','28 ta 1 rx','26 ta 27 rx'],
  2084:['25 ta 6 rx','24 ta 33 rx','24 ta 59','26 ta 20','28 ta 15','0 ge 29','2 ge 30','4 ge 5','4 ge 51','4 ge 37 rx','3 ge 29 rx','1 ge 53 rx'],
  2085:['0 ge 25 rx','29 ta 43 rx','0 ge 1','1 ge 17','3 ge 11','5 ge 29','7 ge 39','9 ge 27','10 ge 26','10 ge 26 rx','9 ge 25 rx','7 ge 50 rx'],
  2086:['6 ge 13 rx','5 ge 20 rx','5 ge 27','6 ge 37','8 ge 30','10 ge 53','13 ge 11','15 ge 12','16 ge 28','16 ge 42 rx','15 ge 53 rx','14 ge 19 rx'],
  2087:['12 ge 35 rx','11 ge 28 rx','11 ge 24','12 ge 25','14 ge 17','16 ge 43','19 ge 11','21 ge 26','23 ge 0','23 ge 32 rx','22 ge 57 rx','21 ge 28 rx'],
  2088:['19 ge 37 rx','18 ge 16 rx','17 ge 59','18 ge 52','20 ge 41','23 ge 11','25 ge 48','28 ge 19','0 cn 11','1 cn 2','0 cn 42 rx','29 ge 20 rx'],
  2089:['27 ge 25 rx','25 ge 51 rx','25 ge 20','25 ge 59','27 ge 42','0 cn 14','3 cn 0','5 cn 47','8 cn 2','9 cn 18','9 cn 22 rx','8 cn 14 rx'],
  2090:['6 cn 18 rx','4 cn 29 rx','3 cn 39 rx','4 cn 0','5 cn 33','8 cn 5','11 cn 0','14 cn 4','16 cn 43','18 cn 28','19 cn 2 rx','18 cn 15 rx'],
  2091:['16 cn 24 rx','14 cn 22 rx','13 cn 10 rx','13 cn 9','14 cn 28','16 cn 55','19 cn 56','23 cn 16','26 cn 21','28 cn 38','29 cn 49','29 cn 33 rx'],
  2092:['27 cn 58 rx','25 cn 47 rx','24 cn 10 rx','23 cn 43','24 cn 43','27 cn 2','0 le 7','3 le 41','7 le 11','10 le 1','11 le 52','12 le 15 rx'],
  2093:['11 le 5 rx','8 le 55 rx','7 le 1 rx','5 le 59 rx','6 le 29','8 le 29','11 le 29','15 le 13','19 le 6','22 le 30','25 le 9','26 le 21'],
  2094:['25 le 56 rx','24 le 4 rx','21 le 57 rx','20 le 17 rx','20 le 7','21 le 35','24 le 20','28 le 3','2 vi 12','6 vi 6','9 vi 31','11 vi 39'],
  2095:['12 vi 12 rx','11 vi 0 rx','8 vi 57 rx','6 vi 45 rx','5 vi 47 rx','6 vi 29','8 vi 43','12 vi 12','16 vi 24','20 vi 38','24 vi 40','27 vi 41'],
  2096:['29 vi 19','29 vi 6 rx','27 vi 26 rx','25 vi 1 rx','23 vi 20 rx','23 vi 7','24 vi 37','27 vi 36','1 li 35','5 li 54','10 li 18','13 li 57'],
  2097:['16 li 32','17 li 22','16 li 32 rx','14 li 22 rx','12 li 9 rx','10 li 58 rx','11 li 27','13 li 36','17 li 3','21 li 9','25 li 38','29 li 41'],
  2098:['2 sc 59','4 sc 49','4 sc 55 rx','3 sc 24 rx','1 sc 9 rx','29 li 13 rx','28 li 40','29 li 48','2 sc 27','6 sc 3','10 sc 19','14 sc 27'],
  2099:['18 sc 10','20 sc 44','21 sc 39','21 sc 0 rx','19 sc 5 rx','16 sc 50 rx','15 sc 28 rx','15 sc 35','17 sc 18','20 sc 13','24 sc 2','28 sc 1'],
  2100:['1 sa 52','4 sa 52','6 sa 24','6 sa 32 rx','5 sa 11 rx','3 sa 0 rx','1 sa 9 rx','0 sa 24','1 sa 13','3 sa 22','6 sa 37','10 sa 16'],
  2101:['14 sa 3','17 sa 15','19 sa 12','19 sa 57','19 sa 12 rx','17 sa 19 rx','15 sa 17 rx','13 sa 57 rx','13 sa 59','15 sa 24','18 sa 2','21 sa 17'],
  2102:['24 sa 52','28 sa 6','0 cp 18','1 cp 30','1 cp 16 rx','29 sa 48 rx','27 sa 48 rx','26 sa 7 rx','25 sa 32','26 sa 18','28 sa 20','1 cp 10'],
  2103:['4 cp 28','7 cp 39','9 cp 58','11 cp 29','11 cp 41 rx','10 cp 36 rx','8 cp 47 rx','6 cp 56 rx','5 cp 55 rx','6 cp 8','7 cp 38','10 cp 2'],
  2104:['13 cp 3','16 cp 7','18 cp 32','20 cp 15','20 cp 44 rx','19 cp 59 rx','18 cp 23 rx','16 cp 30 rx','15 cp 13 rx','15 cp 3','16 cp 6','18 cp 8'],
  2105:['20 cp 52','23 cp 46','26 cp 6','27 cp 58','28 cp 42','28 cp 17 rx','26 cp 54 rx','25 cp 4 rx','23 cp 36 rx','23 cp 5','23 cp 45','25 cp 25'],
  2106:['27 cp 52','0 aq 36','2 aq 54','4 aq 51','5 aq 48','5 aq 38 rx','4 aq 30 rx','2 aq 46 rx','1 aq 11 rx','0 aq 25 rx','0 aq 43','2 aq 4'],
  2107:['4 aq 15','6 aq 49','9 aq 4','11 aq 4','12 aq 10','12 aq 15 rx','11 aq 19 rx','9 aq 43 rx','8 aq 5 rx','7 aq 7 rx','7 aq 8','8 aq 11'],
  2108:['10 aq 8','12 aq 32','14 aq 48','16 aq 48','18 aq 0','18 aq 14 rx','17 aq 29 rx','15 aq 59 rx','14 aq 22 rx','13 aq 16 rx','13 aq 5','13 aq 55'],
  2109:['15 aq 39','17 aq 54','20 aq 1','22 aq 2','23 aq 20','23 aq 44 rx','23 aq 10 rx','21 aq 49 rx','20 aq 12 rx','19 aq 0 rx','18 aq 36','19 aq 13'],
  2110:['20 aq 44','22 aq 50','24 aq 53','26 aq 54','28 aq 17','28 aq 50','28 aq 25 rx','27 aq 12 rx','25 aq 38 rx','24 aq 21 rx','23 aq 47 rx','24 aq 12'],
  2111:['25 aq 31','27 aq 29','29 aq 27','1 pi 28','2 pi 55','3 pi 35','3 pi 20 rx','2 pi 15 rx','0 pi 44 rx','29 aq 24 rx','28 aq 41 rx','28 aq 54'],
  2112:['0 pi 3','1 pi 53','3 pi 51','5 pi 51','7 pi 19','8 pi 5','7 pi 56 rx','6 pi 58 rx','5 pi 30 rx','4 pi 8 rx','3 pi 20 rx','3 pi 25'],
  2113:['4 pi 26','6 pi 8','7 pi 58','9 pi 57','11 pi 28','12 pi 20','12 pi 19 rx','11 pi 28 rx','10 pi 4 rx','8 pi 42 rx','7 pi 47 rx','7 pi 43'],
  2114:['8 pi 34','10 pi 9','11 pi 55','13 pi 53','15 pi 27','16 pi 24','16 pi 31 rx','15 pi 47 rx','14 pi 27 rx','13 pi 4 rx','12 pi 3 rx','11 pi 52'],
  2115:['12 pi 34','14 pi 2','15 pi 44','17 pi 41','19 pi 17','20 pi 19','20 pi 32 rx','19 pi 55 rx','18 pi 40 rx','17 pi 17 rx','16 pi 12 rx','15 pi 53'],
  2116:['16 pi 27','17 pi 48','19 pi 30','21 pi 25','23 pi 2','24 pi 8','24 pi 26 rx','23 pi 54 rx','22 pi 43 rx','21 pi 20 rx','20 pi 12 rx','19 pi 48'],
  2117:['20 pi 16','21 pi 31','23 pi 6','25 pi 0','26 pi 39','27 pi 49','28 pi 13 rx','27 pi 48 rx','26 pi 41 rx','25 pi 19 rx','24 pi 8 rx','23 pi 37 rx'],
  2118:['23 pi 57','25 pi 7','26 pi 38','28 pi 31','0 ar 11','1 ar 25','1 ar 55','1 ar 36 rx','0 ar 34 rx','29 pi 14 rx','27 pi 59 rx','27 pi 22 rx'],
  2119:['27 pi 36','28 pi 39','0 ar 6','1 ar 57','3 ar 39','4 ar 57','5 ar 33','5 ar 21 rx','4 ar 24 rx','3 ar 4 rx','1 ar 47 rx','1 ar 4 rx'],
  2120:['1 ar 11','2 ar 8','3 ar 35','5 ar 25','7 ar 7','8 ar 28','9 ar 8','9 ar 1 rx','8 ar 8 rx','6 ar 49 rx','5 ar 30 rx','4 ar 44 rx'],
};
...
function openAstroWheelModal(wrap) {
  ...
  root.innerHTML = '<div class="astro-wheel-modal-backdrop" data-astro-wheel-modal-close></div><section class="astro-wheel-modal-panel"><button class="astro-wheel-modal-close" data-astro-wheel-modal-close type="button">Volver a la lectura</button></section>';
...
function astroFormDefaults() {
  const profile = getProfile();
  const place = getBirthPlace() || profile.birthPlace || null;
  return {
    name:localStorage.getItem(LS.name) || '',
    birth:getBirthDate() || profile.birth || '',
    time:getBirthTime() || profile.birthTime || '',
    place,
    intention:localStorage.getItem(LS.intention) || profile.intention || '',
...
function astroSharedFormHTML(mode = 'natal') {
  const defaults = astroFormDefaults();
  const n = escapeHTML(defaults.name);
  const d = escapeHTML(defaults.birth);
  const tm = escapeHTML(defaults.time);
  ...
  return `
    ...
    <div class="form-grid mt astro-form">
      <div class="field"><label for="astroName">${escapeHTML(t('nuName'))}</label>${inputWithMic('astroName', `value="${n}" placeholder="${escapeHTML(t('nuNamePh'))}"`)}</div>
      <div class="field"><label for="astroDate">${escapeHTML(t('nuBirth'))}</label><input id="astroDate" class="input" type="date" value="${d}"></div>
      <div class="field"><label for="astroTime">${escapeHTML(t('nuBirthTime'))}</label><input id="astroTime" class="input" type="time" value="${tm}"><small class="subtle">${escapeHTML(t('nuBirthTimePh'))}</small></div>
      <div class="field astro-place-field"><label for="astroPlace">${escapeHTML(t('asPlace'))}</label>${inputWithMic('astroPlace', `value="${placeValue}" placeholder="${escapeHTML(t('asPlacePh'))}" autocomplete="off" aria-describedby="astroPlaceHelp"`)}<input id="astroPlaceData" type="hidden" value="${placeData}"><small id="astroPlaceHelp" class="subtle">${escapeHTML(t('asPlaceHelp'))}</small><div id="astroPlaceSuggestions" class="astro-city-suggestions" aria-live="polite"></div></div>
      <div class="field"><label for="astroHouseSystem">${escapeHTML(t('asHouses'))}</label><select id="astroHouseSystem" class="input"><option value="placidus" ${houseSystem === 'placidus' ? 'selected' : ''}>${escapeHTML(t('asHousesPlacidus'))}</option><option value="quadrant" ${houseSystem === 'quadrant' ? 'selected' : ''}>${escapeHTML(t('asHousesQuadrant'))}</option><option value="equal" ${houseSystem === 'equal' ? 'selected' : ''}>${escapeHTML(t('asHousesEqual'))}</option><option value="whole" ${houseSystem === 'whole' ? 'selected' : ''}>${escapeHTML(t('asHousesWhole'))}</option></select></div>
      <div class="field"><label for="astroIntention">${escapeHTML(t('asIntent'))}</label>${inputWithMic('astroIntention', `value="${escapeHTML(defaults.intention)}" placeholder="${escapeHTML(t('asIntentPh'))}"`)}</div>
      ${extra}
    </div>
    <div class="actions mt"><button class="btn" data-act="astro-menu" type="button">Volver a Astros</button><button class="btn primary" data-act="${action}" type="button">${label}</button></div>
...
function astroPeriodCards(chart, periodChart, intention = '', seedExtra = '') {
  const seed = astroHash(`${chart.name}|${chart.date}|${chart.time}|${periodChart.date}|${periodChart.time}|${intention}|${seedExtra}`);
  const pool = [
    { title:'Puerta', planet:periodChart.planets[(seed + 2) % periodChart.planets.length], text:'marca el tono de entrada para esta consulta' },
    { title:'Foco', planet:periodChart.planets[(seed + 4) % periodChart.planets.length], text:'señala dónde conviene poner claridad' },
    { title:'Ajuste', planet:periodChart.planets[(seed + 7) % periodChart.planets.length], text:'muestra la tensión que pide orden y calma' },
    { title:'Acción', planet:periodChart.planets[(seed + 9) % periodChart.planets.length], text:'propone un gesto concreto y realizable' }
  ];
  return pool.map(card => ({ ...card, sign:card.planet.sign, symbol:astroGlyph(card.planet.symbol), element:card.planet.element }));
}
...
function parseAstroMonth(value = astroMonthValue()) {
  ...
  const month = match ? Math.max(1, Math.min(12, Number(match[2]))) : now.getMonth() + 1;
  return { year, month, value:`${year}-${String(month).padStart(2, '0')}` };
}
function astroMonthMarkers(data, place, houseSystem) {
  const parsed = parseAstroMonth(data.month);
  const daysInMonth = new Date(parsed.year, parsed.month, 0).getDate();
  const days = [1, 8, 15, 22].map(day => Math.min(day, daysInMonth));
  return days.map((day, index) => {
    const date = `${parsed.value}-${String(day).padStart(2, '0')}`;
    const chart = calculateAstroProfile(data.name, date, '12:00', place, { houseSystem });
    const labels = ['Inicio', 'Impulso', 'Centro', 'Cierre'];
    return { title:labels[index], date, chart, symbol:astroGlyph(chart.moon.signSymbol), sign:chart.moon.sign, element:chart.moon.element };
  });
}
function monthlyAstroText(chart, monthChart, markers, data) {
  const parsed = parseAstroMonth(data.month);
  return `INFORME ASTRAL MENSUAL · ${chart.name}
Nacimiento: ${chart.date} · Hora natal: ${chart.time}
Lugar: ${chart.place?.label || 'No indicado'}
Mes consultado: ${parsed.value}
Intención: ${data.intention || 'Claridad'}
Motor: ${chart.engine}
Sistema de casas: ${astroHouseSystemLabel(chart.houseSystem)}
...
Rueda de referencia mensual:
Sol en ${monthChart.sun.name}. Luna en ${monthChart.moon.sign}. Ascendente del mes en ${monthChart.asc.name}. Medio Cielo del mes en ${monthChart.mc.name}.

Momentos del mes:
${markers.map(item => `${item.title} (${item.date}): Luna en ${item.sign}. Consejo: ${astroAdviceForElement(item.element)}.`).join('\n')}

Síntesis:
Este mes se lee en cuatro pasos para no mezclarlo todo en una sola página: entrada, impulso, centro y cierre. La lectura orienta prioridades y no sustituye una planificación profesional.`;
}
async function monthlyAstroReading() {
  const data = getAstroFormData();
  if (!data.name || !data.date || !data.time || !data.place?.label) return toast(t('tsBirthMissing'));
  data.place = await resolveAstroPlace(data.place, data.place?.label);
  const chart = calculateAstroProfile(data.name, data.date, data.time, data.place, { houseSystem:data.houseSystem });
  ...
  persistAstroData(data);
  const parsed = parseAstroMonth(data.month);
  const monthChart = calculateAstroProfile(data.name, `${parsed.value}-15`, '12:00', data.place, { houseSystem:data.houseSystem });
  const markers = astroMonthMarkers(data, data.place, data.houseSystem);
  const cards = markers.map(item => ({ title:item.title, planet:item.chart.moon, sign:item.sign, symbol:item.symbol, element:item.element, text:`Luna en ${item.sign} el ${item.date}` }));
  const title = `Mes astral ${parsed.value} · ${data.name}`;
  const subtitle = `${parsed.value} · ${data.place.label}`;
  const text = monthlyAstroText(chart, monthChart, markers, data);
  setLastReading({ type:'Astros', title, text, items:cards.map(card => ({ kind:'astro', name:`${card.title}: Luna en ${card.sign}`, subtitle:card.element, image:'', symbol:card.symbol, position:card.title })), meta:{ name:data.name, birthDate:data.date, birthTime:data.time, birthPlace:data.place, intention:data.intention, houseSystem:data.houseSystem, astro:chart, astroPeriod:{ kind:'month', label:parsed.value, month:parsed.value, markers, chart:monthChart } } });
  renderAstroPeriodReading(chart, monthChart, cards, text, title, subtitle);
}
...
function grabovoiPanelHTML() {
  ...
  return `
    ...
    <div class="field mt"><label for="grabSearch">${escapeHTML(t('gbSearch'))}</label>${inputWithMic('grabSearch', `placeholder="${escapeHTML(t('gbSearchPh'))}"`)}</div>
    ${renderGrabChips()}
...
async function showNumerologyGrabovoiPdfPicker() {
  ...
  openModal({ icon:'📜', title:t('gbPdfTitle'), subtitle:subject ? `${t('lblFor')} ${subject}` : t('gbPdfPick'), body:`
    ...
    <div class="field mt"><label for="grabPdfSearch">${escapeHTML(t('gbPdfSearch'))}</label>${inputWithMic('grabPdfSearch', `placeholder="${escapeHTML(t('gbPdfSearchPh'))}"`)}</div>
...
function showSettings() {
  ...
  openModal({ icon:'⚙️', title:t('mdAjustes'), subtitle:t('mdAjustesS'), body:`
    ...
    <div class="form-grid">
      <div class="field"><label for="appLanguage">${escapeHTML(t('appLanguage'))}</label><select id="appLanguage">${languageOptionsHTML()}</select><small>${escapeHTML(t('languageHelp'))}</small></div>
      <div class="field"><label for="settingsName">${escapeHTML(t('stNombreOpcional'))}</label>${inputWithMic('settingsName', `value="${name}" placeholder="${escapeHTML(t('phTuNombre'))}"`)}</div>
      <div class="field"><label for="settingsAiStatus">${escapeHTML(t('stEstadoIa'))}</label><input id="settingsAiStatus" class="input" value="${escapeHTML(t(localStorage.getItem(LS.puter)==='true'?'stIaConectada':'stIaSimbolica'))}" readonly></div>
      <div class="field"><label for="aiStyle">${escapeHTML(t('stTipoDeLecturaIa'))}</label><select id="aiStyle"><option value="mistica" ${getAIStyle() === 'mistica' ? 'selected' : ''}>${escapeHTML(t('stMistica'))}</option><option value="razonable" ${getAIStyle() === 'razonable' ? 'selected' : ''}>${escapeHTML(t('stRazonable'))}</option><option value="corta" ${getAIStyle() === 'corta' ? 'selected' : ''}>${escapeHTML(t('stCorta'))}</option><option value="profunda" ${getAIStyle() === 'profunda' ? 'selected' : ''}>${escapeHTML(t('stProfunda'))}</option><option value="directa" ${getAIStyle() === 'directa' ? 'selected' : ''}>${escapeHTML(t('stDirecta'))}</option><option value="amorosa" ${getAIStyle() === 'amorosa' ? 'selected' : ''}>${escapeHTML(t('stAmorosa'))}</option></select></div>
    ...
    <div class="settings-section-content">
    <div class="form-grid">
      <div class="field"><label for="voiceEngine">${escapeHTML(t('stMotorDeVoz'))}</label><select id="voiceEngine"><option value="device" ${voice.engine === 'device' ? 'selected' : ''}>${escapeHTML(t('stTtsDelSistemaSinPuter'))}</option><option value="auto" ${voice.engine === 'auto' ? 'selected' : ''}>${escapeHTML(t('stAutomaticoIaSiEstaConectada'))}</option><option value="puter" ${voice.engine === 'puter' ? 'selected' : ''}>${escapeHTML(t('stVozIaNaturalMediantePuter'))}</option></select></div>
      <div class="field"><label for="remoteVoice">${escapeHTML(t('stVozIaRemota'))}</label><select id="remoteVoice"><option value="coral" ${voice.remoteVoice === 'coral' ? 'selected' : ''}>${escapeHTML(t('stCoralFemeninaCalida'))}</option><option value="sage" ${voice.remoteVoice === 'sage' ? 'selected' : ''}>${escapeHTML(t('stSageFemeninaSerena'))}</option><option value="onyx" ${voice.remoteVoice === 'onyx' ? 'selected' : ''}>${escapeHTML(t('stOnyxMasculinaProfunda'))}</option><option value="ash" ${voice.remoteVoice === 'ash' ? 'selected' : ''}>${escapeHTML(t('stAshMasculinaNatural'))}</option></select></div>
      <div class="field"><label for="voiceFilter">${escapeHTML(t('stCatalogoDelSistema'))}</label><select id="voiceFilter"><option value="all" ${(voice.voiceFilter || 'all') === 'all' ? 'selected' : ''}>${escapeHTML(t('stTodasLasVocesVisibles'))}</option><option value="spanish" ${voice.voiceFilter === 'spanish' ? 'selected' : ''}>${escapeHTML(t('stSoloVocesEspanolas'))}</option></select></div>
      <div class="field"><label for="voiceLanguage">${escapeHTML(t('stIdiomaDeVoz'))}</label><select id="voiceLanguage">${getVoiceLanguageOptionsHTML(voice.language || 'auto')}</select></div>
      <div class="field"><label for="deviceVoiceURI">${escapeHTML(t('stVozConcretaDelDispositivo'))}</label><select id="deviceVoiceURI">${getDeviceVoiceOptionsHTML(voice.deviceVoiceURI || '')}</select></div>
      <div class="field"><label for="voicePreset">${escapeHTML(t('stVozMistica'))}</label><select id="voicePreset"><option value="mistica_femenina" ${voice.preset === 'mistica_femenina' ? 'selected' : ''}>${escapeHTML(t('stMisticaFemenina'))}</option><option value="guia_suave" ${voice.preset === 'guia_suave' ? 'selected' : ''}>${escapeHTML(t('stGuiaSuave'))}</option><option value="oraculo_neutro" ${voice.preset === 'oraculo_neutro' ? 'selected' : ''}>${escapeHTML(t('stOraculoNeutro'))}</option><option value="sabio_masculino" ${voice.preset === 'sabio_masculino' ? 'selected' : ''}>${escapeHTML(t('stSabioMasculino'))}</option><option value="guardian_profundo" ${voice.preset === 'guardian_profundo' ? 'selected' : ''}>${escapeHTML(t('stGuardianProfundo'))}</option><option value="lectura_rapida" ${voice.preset === 'lectura_rapida' ? 'selected' : ''}>${escapeHTML(t('stLecturaRapida'))}</option></select></div>
      <div class="field"><label for="voiceRate">${escapeHTML(t('stVelocidadDeVoz'))}</label><select id="voiceRate"><option value="0.78" ${String(voice.rate) === '0.78' ? 'selected' : ''}>${escapeHTML(t('stCeremonial'))}</option><option value="0.88" ${String(voice.rate) === '0.88' ? 'selected' : ''}>${escapeHTML(t('stHumanaSuave'))}</option><option value="0.92" ${String(voice.rate) === '0.92' ? 'selected' : ''}>${escapeHTML(t('stNatural'))}</option><option value="1.04" ${String(voice.rate) === '1.04' ? 'selected' : ''}>${escapeHTML(t('stAgil'))}</option></select></div>
      <div class="field"><label for="keepScreenAwake">${escapeHTML(t('stPantallaDuranteLaVoz'))}</label><select id="keepScreenAwake"><option value="true" ${voice.keepScreenAwake !== false ? 'selected' : ''}>${escapeHTML(t('stMantenerEncendida'))}</option><option value="false" ${voice.keepScreenAwake === false ? 'selected' : ''}>${escapeHTML(t('stPermitirApagadoAutomatico'))}</option></select></div>
    </div>
    ...
    <div class="settings-section-content">
    <div class="form-grid">
      <div class="field"><label for="ceremonySpeed">${escapeHTML(t('stVelocidadDeRevelacion'))}</label><select id="ceremonySpeed"><option value="slow" ${ceremony.speed==='slow'?'selected':''}>${escapeHTML(t('stLentaCeremonial'))}</option><option value="normal" ${(ceremony.speed||'normal')==='normal'?'selected':''}>${escapeHTML(t('stNormal'))}</option><option value="fast" ${ceremony.speed==='fast'?'selected':''}>${escapeHTML(t('stRapida'))}</option></select></div>
      <div class="field"><label for="ceremonySounds">${escapeHTML(t('sndTitle'))}</label><select id="ceremonySounds"><option value="false" ${!ceremony.sounds?'selected':''}>${escapeHTML(t('sndOff'))}</option><option value="true" ${ceremony.sounds?'selected':''}>${escapeHTML(t('sndOn'))}</option></select><small>${escapeHTML(t('sndHelp'))}</small></div>
      <div class="field"><label for="ceremonyVibration">${escapeHTML(t('sndVib'))}</label><select id="ceremonyVibration"><option value="false" ${!ceremony.vibration?'selected':''}>${escapeHTML(t('sndVibOff'))}</option><option value="true" ${ceremony.vibration?'selected':''}>${escapeHTML(t('sndVibOn'))}</option></select></div>
    </div></div></details>
    ...
    <div class="settings-section-content">
    <div class="form-grid">
      <div class="field"><label for="themeSelect">${escapeHTML(t('stTemaVisual'))}</label><select id="themeSelect"><option value="gold" ${getTheme()==='gold'?'selected':''}>${escapeHTML(t('stDoradoMistico'))}</option><option value="violet" ${getTheme()==='violet'?'selected':''}>${escapeHTML(t('stNocheVioleta'))}</option><option value="forest" ${getTheme()==='forest'?'selected':''}>${escapeHTML(t('stBosqueRunico'))}</option><option value="blue" ${getTheme()==='blue'?'selected':''}>${escapeHTML(t('stLunaAzul'))}</option><option value="classic" ${getTheme()==='classic'?'selected':''}>${escapeHTML(t('stTarotClasico'))}</option><option value="light" ${getTheme()==='light'?'selected':''}>${escapeHTML(t('stClaroElegante'))}</option></select></div>
      <div class="field"><label for="appearanceModeSelect">${escapeHTML(t('stLuzDeLaApp'))}</label><select id="appearanceModeSelect" data-om-appearance-select><option value="dark" ${getAppearanceMode()==='dark'?'selected':''}>${escapeHTML(t('stNoche'))}</option><option value="light" ${getAppearanceMode()==='light'?'selected':''}>${escapeHTML(t('stDia'))}</option></select></div>
      <div class="field"><label for="privateModeSelect">${escapeHTML(t('stModoPrivado'))}</label><select id="privateModeSelect"><option value="false" ${!isPrivateMode()?'selected':''}>${escapeHTML(t('stGuardarHistorialNormalmente'))}</option><option value="true" ${isPrivateMode()?'selected':''}>${escapeHTML(t('stNoGuardarNuevasLecturas'))}</option></select></div>
...
${t('dlAdvice')}: ${t('dlAdviceText')}`;
  ...
  openModal({ icon:'🌟', title:t('dailyMessage'), subtitle:t('dlSub'), body:`
    ...
    <div class="result-card mt daily-journal-card"><h3>${escapeHTML(t('dlJournal'))}</h3><div class="form-grid"><div class="field"><label for="dailyMood">${escapeHTML(t('dlMood'))}</label><select id="dailyMood">${animos.map(([k,v]) => `<option value="${v}">${escapeHTML(t(k))}</option>`).join('')}</select></div><div class="field"><label for="dailyIntention">${escapeHTML(t('dlIntent'))}</label><input class="input" id="dailyIntention" placeholder="${escapeHTML(t('dlIntentPh'))}"></div></div><div class="field mt"><label for="dailyReflection">${escapeHTML(t('dlReflection'))}</label><textarea id="dailyReflection" placeholder="${escapeHTML(t('dlReflectionPh'))}"></textarea></div><div class="actions mt"><button class="btn primary" data-act="save-daily-reflection">${escapeHTML(t('dlSave'))}</button><button class="btn" data-act="daily-history">${escapeHTML(t('dlHistory'))}</button></div></div>` });
}
