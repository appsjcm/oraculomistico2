...
const bind=()=>{const search=$("#premiumRealCardSearch");if(search)search.addEventListener("input",()=>{activeQuery=search.value;resetPaging();renderCards()});$$(".engine-filter").forEach(b=>b.addEventListener("click",()=>{activeFilter=b.dataset.cardFilter||"all";$$('.engine-filter').forEach(x=>x.classList.toggle('active',x===b));resetPaging();renderCards()}));$$('[data-native-spread]').forEach(b=>b.addEventListener('click',()=>{if(!getSettings().nativeEngine){toast('Activa el motor local real en Ajustes.');return}drawSpread(Number(b.dataset.nativeSpread||'1'))}));document.addEventListener('click',ev=>{const action=ev.target?.closest?.('[data-premium-action]')?.dataset?.premiumAction;if(action==='save-native-spread')saveSpread();if(action==='render-native-vault'){renderVault();toast('Vault actualizado.')}if(action==='clear-native-vault')clearVault();if(action==='open-native-engine')scrollEngine();if(action==='load-more-cards'){shownCards+=PAGE_SIZE;renderCards()}});const n=$('#premiumNativeEngineToggle'),c=$('#premiumCompactCards');if(n)n.addEventListener('change',()=>saveSettings({nativeEngine:n.checked}));if(c)c.addEventListener('change',()=>saveSettings({compactCards:c.checked}))};
