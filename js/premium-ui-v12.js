...
const filteredCards=()=>{const q=activeQuery.trim().toLowerCase();return CARDS.filter(c=>{const mf=activeFilter==="all"||c.type===activeFilter;const hay=`${c.name} ${c.type} ${c.suit} ${c.keywords}`.toLowerCase();return mf&&(!q||hay.includes(q))})};
const renderCards=()=>{const grid=$("#premiumRealCardGrid");if(!grid)return;const list=filteredCards();const count=$("#premiumCardCount"),current=$("#premiumCurrentFilter");if(count)count.textContent=String(list.length);if(current)current.textContent=label(activeFilter);if(!list.length){grid.innerHTML='<div class="premium-empty-state"><strong>No hay cartas con ese filtro.</strong><span>Prueba otra búsqueda o vuelve a “Todas”.</span></div>';return}grid.innerHTML=list.map(c=>`<article class="premium-real-card" data-card-id="${escapeHTML(c.id)}" data-card-type="${escapeHTML(c.type)}"><div class="card-sigil">${escapeHTML(c.symbol)}</div><h3>${escapeHTML(c.name)}</h3><p>${escapeHTML(c.keywords)}</p><div class="card-meta"><span>${escapeHTML(c.suit)}</span><span>${escapeHTML(c.type==="mayor"?"Arcano Mayor":"Arcano Menor")}</span></div></article>`).join("")};
...
const drawSpread=(count)=>{const r=$("#premiumSpreadResult");if(!r)return;const drawn=shuffle(CARDS).slice(0,count);lastSpread={id:`spread-${Date.now()}`,createdAt:new Date().toISOString(),type:spreadLabel(count),cards:drawn};r.innerHTML=drawn.map((c,i)=>`<article class="premium-drawn-card"><span class="draw-position">${escapeHTML(pos(count,i))}</span><h3>${escapeHTML(c.name)}</h3><p>${escapeHTML(c.keywords)}</p></article>`).join("");toast(`Tirada generada: ${spreadLabel(count)}`)};
...
const bind=()=>{const search=$("#premiumRealCardSearch");if(search)search.addEventListener("input",()=>{activeQuery=search.value;renderCards()});$$(".engine-filter").forEach(b=>b.addEventListener("click",()=>{activeFilter=b.dataset.cardFilter||"all";$$('.engine-filter').forEach(x=>x.classList.toggle('active',x===b));renderCards()}));$$('[data-native-spread]').forEach(b=>b.addEventListener('click',()=>{if(!getSettings().nativeEngine){toast('Activa el motor local real en Ajustes.');return}drawSpread(Number(b.dataset.nativeSpread||'1'))}));document.addEventListener('click',ev=>{const action=ev.target?.closest?.('[data-premium-action]')?.dataset?.premiumAction;if(action==='save-native-spread')saveSpread();if(action==='render-native-vault'){renderVault();toast('Vault actualizado.')}if(action==='clear-native-vault')clearVault();if(action==='open-native-engine')scrollEngine()});const n=$('#premiumNativeEngineToggle'),c=$('#premiumCompactCards');if(n)n.addEventListener('change',()=>saveSettings({nativeEngine:n.checked}));if(c)c.addEventListener('change',()=>saveSettings({compactCards:c.checked}))};
const init=()=>{applySettings();bind();renderCards();renderVault();updateStats()};document.readyState==='loading'?document.addEventListener('DOMContentLoaded',init,{once:true}):init();
})();
